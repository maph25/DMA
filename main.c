...
void DMA_init(void)
{
	...
	DMA0->TCD[0].DOFF = 5;/*destination address signed offset;it is expressed in number of bytes
	...
	DMA0->TCD[0].SLAST = -9;//restores the source address to the initial value, which is expressed in the amount of bytes to restore*/
